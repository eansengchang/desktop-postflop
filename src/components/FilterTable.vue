            ...
            >
              ...
              <template v-else-if="column.type === 'bar'">
                <div
                  class="w-full h-full bg-neutral-800 bg-left bg-no-repeat"
                  :style="{
                    'background-image': strategyBarBgImage(item[1]),
                    'background-size': strategyBarBgSize(item[1]),
                  }"
                ></div>
              </template>

              <template v-else-if="isNaN(item[1][columnIndex(column)])">
                <span>-</span>
              </template>
              ...
              <template v-else>
                <div class="inline-block w-12 text-right">
                  <span
                    ...
                    :data-set="
                      (strTmp = toFixedAdaptive(item[1][columnIndex(column)]))
                    "
                  >
                    <span>{{ strTmp.slice(0, strTmp.indexOf(".") + 1) }}</span>
                    <span class="text-xs">{{
                  ...
                  <span
                    ...
                    "
                    :data-set="
                      (strTmp = toFixed1(item[1][columnIndex(column)] * 100))
                    "
                  >
                    <span>{{ strTmp.slice(0, -1) }}</span>
                    <span class="text-xs">{{ strTmp.slice(-1) }}%</span>
...
const strategyBarBgImage = (row: number[] | undefined) => {
...
const strategyBarBgSize = (row: number[] | undefined) => {
  if(!row) return "100% 100%"; 
